...
function certToPEM(cert) {
	...
	if (cert != null && cert.length == 65 && cert[0] == 0x04) {
		...
		asn1key = jsrsasign.b64toBA(jsrsasign.hextob64("3059301306072a8648ce3d020106082a8648ce3d030107034200"));
...
function resolveCredentialIdBytesFromPrivateKeyHex(privKeyHEX) {
	...
	return jsrsasign.b64toBA(CryptoJS.AES.encrypt(privKeyHEX, fidoutilsConfig.encryptionPassphrase).toString());
}
...
function resolvePrivateKeyHexFromCredentialIdBytes(credIdBytes) {
	...
	return CryptoJS.AES.decrypt(jsrsasign.hextob64(jsrsasign.BAtohex(credIdBytes)), fidoutilsConfig.encryptionPassphrase).toString(CryptoJS.enc.Utf8);
}
...
function attestationOptionsResponeToCredentialCreationOptions(o) {
	...
	pkcco.user.id = new Uint8Array(jsrsasign.b64toBA(jsrsasign.b64utob64(o.user.id)));
	 ...
	 */
	pkcco.challenge = new Uint8Array(jsrsasign.b64toBA(jsrsasign.b64utob64(o.challenge)));
	...
	if (o["excludeCredentials"] != null) {
		...
		for (let i = 0; i < o.excludeCredentials.length; i++) {
			...
			// required: id in o is base64url, but in pkcco is BufferSource
			c.id = new Uint8Array(jsrsasign.b64toBA(jsrsasign.b64utob64(o.excludeCredentials[i].id)));
...
function processCredentialCreationOptions(cco, attestationFormat = 'none', up = true, uv = true) {
	...
	let clientDataJSON = {
			...
			"challenge": jsrsasign.hextob64u(jsrsasign.BAtohex(bytesFromArray(cco.publicKey.challenge,0,-1))),
	...
	let clientDataHash = sha256(jsrsasign.b64toBA(jsrsasign.utf8tob64(JSON.stringify(clientDataJSON))));
	 ...
	 */
	let flags = (up ? 0x01 : 0x00) | ((uv && attestationFormat != 'fido-u2f') ? 0x04 : 0x00) | 0x40;
	authData.push(flags);
	...
	let aaguid = ((fidoutilsConfig[attestationFormat] == null || fidoutilsConfig[attestationFormat].aaguid == null) ? null : jsrsasign.b64toBA(jsrsasign.hextob64(fidoutilsConfig[attestationFormat].aaguid.replace(/-/g,""))));
	if (aaguid == null) {
	...
	//
	let credIdBytes = resolveCredentialIdBytesFromPrivateKeyHex(keypair.prvKeyObj.prvKeyHex);
	...
	let credPublicKeyCOSE = {
			...
			"3":-7, // alg
			"-1":1, // crv
			"-2": jsrsasign.b64toBA(jsrsasign.hextob64(keypair.pubKeyObj.getPublicKeyXYHex()["x"])), // xCoordinate
			"-3": jsrsasign.b64toBA(jsrsasign.hextob64(keypair.pubKeyObj.getPublicKeyXYHex()["y"])) // yCoordinate
	};
	
	// credentialIdLength (2 bytes) and credential Id
	let lenArray = [ (credIdBytes.length - (credIdBytes.length & 0xFF)) / 256, credIdBytes.length & 0xFF];
	...
	let credPublicKeyBytes = bytesFromArray(new Uint8Array(cbor.encode(credPublicKeyCOSE)), 0, -1);
	...
	let attestationObject = {"fmt": attestationFormat, "attStmt": attStmt, "authData": authData };
	
	
	// add the base64url of the CBOR encoding of the attestationObject to the response
	saar.attestationObject = jsrsasign.hextob64u(jsrsasign.BAtohex(bytesFromArray(new Uint8Array(cbor.encode(attestationObject)), 0, -1)));
...
function assertionOptionsResponeToCredentialRequestOptions(o) {
	 ...
	 */
	pkcro.challenge = new Uint8Array(jsrsasign.b64toBA(jsrsasign.b64utob64(o.challenge)));
	...
	if (o["allowCredentials"] != null) {
		...
		for (let i = 0; i < o.allowCredentials.length; i++) {
			...
			// required: id in o is base64url, but in pkcco is BufferSource
			c.id = new Uint8Array(jsrsasign.b64toBA(jsrsasign.b64utob64(o.allowCredentials[i].id)));
			// optional: transports - copy across if present
 ...
 */
function processCredentialRequestOptions(cro, authenticatorRecords, up = true, uv = true, payloadHash = null) {
	...
	let clientDataJSON = {
			"origin": fidoutilsConfig.origin,
			"challenge": jsrsasign.hextob64u(jsrsasign.BAtohex(bytesFromArray(cro.publicKey.challenge,0,-1))),
			"type": "webauthn.get"
	};
	...
	if (cro.publicKey["allowCredentials"] != null && cro.publicKey["allowCredentials"].length > 0) {
		for (let i = 0; i < cro.publicKey["allowCredentials"].length && privKeyHex == null; i++) {
			let candidateCredIdBytes = bytesFromArray(cro.publicKey["allowCredentials"][i].id, 0, -1);
			let candidateCredIdStr =jsrsasign.hextob64u(jsrsasign.BAtohex(candidateCredIdBytes));
			try { 
				let candidatePrivKeyHex = resolvePrivateKeyHexFromCredentialIdBytes(candidateCredIdBytes);
		...
		for (let i = 0; i < allCredentialIDs.length & usedCredentialId == null; i++) {
...
function buildFidoU2FAttestationStatement(keypair, clientDataHash, authData, credIdBytes) {
	...
	attestationCert.readCertPEM(certToPEM(jsrsasign.b64toBA(fidoutilsConfig["fido-u2f"].cert)));
	...
	let sigBase = [ 0x00 ].concat(
			...
			credIdBytes, jsrsasign.b64toBA(jsrsasign.hextob64(keypair.pubKeyObj.pubKeyHex)));
...
function buildPackedAttestationStatement(keypair, clientDataHash, authData, credIdBytes, useSelfAttestation) {
	...
	if (useSelfAttestation) {
		...
		let attestationCert = new jsrsasign.X509();
		attestationCert.readCertPEM(certToPEM(jsrsasign.b64toBA(fidoutilsConfig.packed.cert)));
		attStmt.x5c = [ jsrsasign.b64toBA(jsrsasign.hextob64(attestationCert.hex)) ];
	}
